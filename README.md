...
----> returns the lowest Card in a given set <br />
	...
	**Idiot**
```func dealerMove()```<br />
----> handles the dealer's move by evaluating the moveManager global variable <br />
```func checkMoveManager()```<br />
----> evaluates the top of the Main Pile and changes moveManager according to its value <br />
```func compareTop(c: Card) -> Int```<br />
----> evaluates if the given card is equal to or greater than the top of the Main Pile using peek <br />
```func setVal(input: String) -> Int```<br />
----> takes in the player's input and returns the appropriate Card Value <br />
```func interactiveMode() ```<br />
----> handles the player's move, calls dealer to respond, runs the game<br />
